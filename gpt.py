...
openai.api_key='' #DON'T PUT THE ACTUAL KEY HERE EVER
load_dotenv()
model = 'gpt-3.5-turbo'
model = 'gpt-4'
...
def listen_for_wake_word(source):
    print("Listening for 'wheatley'...")

    while True:
        audio = r.listen(source)
        try:
            text = r.recognize_google(audio)
            if "wheatley" in text.lower():
...
def listen_and_respond(source):
    ...
    while True:
        ...
        try:
            ...
            # Speak the response
            print("speaking")
            engine.say(response_text) #Maybe engine.say isn't working at all
            engine.runAndWait()
            if not audio:
